...
const DynamicTable = ({ title, data, btnAction, headers, isBtnNeeded }) => {
  ...
  const filteredData = data.filter((item) =>
    headers.some((header) =>
      item[header.key]?.toString().toLowerCase().includes(search.toLowerCase())
    )
  );

  const sortedData = [...filteredData]; // Add sorting logic if needed
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-[#f76411]">{title}</h2>
        <div className="flex items-center gap-2 text-[#f76411]">
          ...
          <div className="relative group  text-[#f76411]">
            <button
              ...
              className="flex items-center space-x-2 text- cursor-pointer border rounded-lg p-1.5"
            >
              <TbPackageExport />
        ...
        <tbody>
          {paginatedProducts.map((row, rowIndex) => (
            <tr
              ...
              } border-t hover:bg-gray-300 hover:text-gray-50 `}
            >
              {headers.map((header, index) => (
                <td key={header.key} className={` p-2 border-r`}>
                  {row[header.key]}
                </td>
              ))}
            </tr>
